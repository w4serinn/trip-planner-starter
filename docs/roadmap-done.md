...
## 85. しおりタイムラインの連結線から緑の線を削除し足あとのみ配置
- [x] (S) しおりタイムラインの連結線(`80`)から緑の線を削除し、足あと
      アイコンのみを軌跡上に配置する形に変更した。2026-08-21、人間から
      「足跡は緑の線はいらない、足跡のみを軌跡上に配置」とのフィードバック。
      `src/views/itinerary.js`の`buildTrailSvg()`から、`createFootprintTrail`
      の戻り値のうち軌跡の線を描画していた`<path d="${pathD}" ...>`要素を
      削除し、足あとの`<g>`群だけを描画するようにした。`pathD`自体の算出
      ロジック(`src/footprintTrail.js`)は変更していない(足あとの位置・
      向きは経路上の等間隔点から算出するため、内部的には引き続き使われて
      いる。呼び出し側で`pathD`を分割代入・使用しなくなっただけ)。
      `pages/shared.css`の`.timeline-trail`は、線が無くなり足あとの点在
      だけになった分、控えめだった不透明度(0.7)を上げると足あとが見えやすく
      なると判断し0.85に調整した。`color: var(--color-success)`(足あとの
      色付け)は引き続き使うため維持した。

      Playwrightで、実Firestore(共有テストグループ`FMXRZYW7`)に対し旅行を
      1件作成し、しおりタブの同じ日に2件の項目を追加した際、`.timeline-
      trail`内に`<path>`要素が存在しないこと(`hasPath: false`)、足あとの
      `<g>`が3件描画されていることを確認した(2026-08-22)。スクリーンショット
      (該当箇所を拡大したものも含む)で、線が完全に消え、緑色の足あとの点が
      軌跡上に単独で点在していることを目視確認した。375px幅で全7タブとも
      横スクロールが発生しないこと(回帰なし)も確認した。console/pageerror
      は0件。`npm run check`(lint・test、vitest 47件)成功。検証で作成した
      Firestore上のトリップ1件はサブコレクションを作成していないため追加
...
## 87. しおりの並び替えを全項目対象に再設計
- [x] (M) 2026-08-21、人間との会話で確定した要望「その日の全項目を対象に自由に
      並び替えられるのが良い、ただ、それにより時間の矛盾が起きる場合のみ警告を
      出す」を実装した。従来は同じ日の中で時間未設定の項目同士でしか▲/▼並び替え
      ができず、「時間が決まっている項目の間に、時間未定の項目を挟みたい」という
      ニーズに応えられなかった。着手前に`docs/firestore-design.md`の「しおり項目
      (時間未設定同士)の並び替え」を「しおり項目の並び替え(全項目対象)」へ書き
      換え、`order`フィールドの新しい意味付け(未着手の日/着手済みの日の2状態)を
      先に確定させてから実装した。

      **設計のポイント(既存データの一括マイグレーション無しで移行できる設計)**:
      各日は「未着手」(全項目`order`未設定)と「着手済み」(全項目に明示的な`order`
      あり)の2状態のみを取り、中間状態は発生しない。並び替え・着手済みの日への
      追加のたびに、対象日の全項目の`order`を`0,1,2,...`へ振り直して保存するため。
      これにより、機能追加前からの既存データはそのまま「未着手」として扱われ、
      人間が実際にその日を触った瞬間に自然に「着手済み」へ移行する。

      **実装内容(`src/views/itinerary.js`)**:
      (1) `compareItems`を、両方`order`未設定なら時刻順(従来通り)、それ以外は
      `order`昇順で比較するよう変更。(2) `moveUntimedItem`を`moveItem`に改名し、
      対象を時間未設定の項目だけでなくその日の全項目に拡張。▲/▼ボタンの表示条件も
      `dayItems.length > 1`(全項目基準)に変更。(3) 新規項目を「着手済みの日」に
      追加する際、その項目の時間目安を既存項目の時刻と比較して挿入位置を求め、
      挿入位置以降の既存項目の`order`をバッチ更新(`updateDocumentsBatch`、`86`で
      追加した関数を再利用)してからバッチ後に新規項目を`order`付きで作成する
      (「未着手の日」への追加は`order`操作不要、追加後も時刻順で正しく表示される
      ため)。(4) 表示順を走査し、それまでの最遅時刻より早い時間を持つ項目を検出して
      `.item-time-warning`(「⚠ 時間の前後が入れ替わっています」、`--color-danger`の
      文字色。新規トークン追加無し)を表示する警告ロジックを追加(保存はブロックしない)。

      Playwrightで、実Firestore(共有テストグループ`FMXRZYW7`)に対し旅行を1件作成し、
      一連のシナリオを検証した(2026-08-21): (a)未着手の日に9:00「朝食」・12:00
      「昼食」・時間未設定「自由時間」を追加すると時刻順(朝食→昼食→自由時間)で
      表示されること、(b)▲ボタンが3件全てに表示されること(時間設定の有無を問わず)、
      (c)「自由時間」を1つ上へ移動すると朝食→自由時間→昼食の順になり、時間未設定の
      項目を時間設定項目の間に挟めること、(d)着手済みの日に10:00「小さな用事」を
      追加すると朝食(9:00)と自由時間の間に自動挿入され朝食→小さな用事→自由時間→
      昼食の順になること、(e)「昼食」(12:00)を2回上へ移動して10:00の項目より前に
      来るよう手動並び替えすると、朝食→昼食→小さな用事→自由時間の順になり
      「小さな用事」に警告(「⚠ 時間の前後が入れ替わっています」)が表示されること、
      をすべて確認した。スクリーンショットで警告表示のスタイルも目視確認した。
      375px幅で全7タブとも横スクロールが発生しないこと(回帰なし)、console/
      pageerrorが0件であることも確認した。`npm run check`(lint・test、vitest
      47件)成功。検証で作成したFirestore上のトリップ1件はサブコレクションを
      作成していないため追加クリーンアップ不要、名前を「[検証用/削除不可]
      evolve cycle6 87検証で作成」に更新済みの状態で共有テストグループ
      `FMXRZYW7`内に残置。

## 89. ボタンサイズ再調整
- [x] (S) `77`・`88`対応後も、人間から「ボタンの大きさ大きすぎない？という
      かこの程度の文字とボタンならこんな大きさでこんな位置に置くべきでは
      ない」との指摘(2026-08-21、しおり項目の▲上へ/▼下へ/編集/削除の
      4ボタンが横幅いっぱいのピル型で並ぶスクリーンショット付き)。
      `.btn-secondary`は`88`で幅(width: auto)のみ主役ボタンと差別化して
      いたが、高さ・padding・font-sizeは無印`button`のまま(min-height:
      44px等)だったため、軽い操作が複数並ぶ箇所では依然大きすぎた。
      `pages/shared.css`の`.btn-secondary`に`min-height: 36px`・
      `padding: var(--space-xs) var(--space-md)`・
      `font-size: var(--font-size-sm)`を追加し、幅だけでなく高さ・太さでも
      主役ボタンとの差を付けた。新規トークン追加無し(既存の
      `--space-xs`/`--space-md`/`--font-size-sm`を使用)。

      Playwrightで、実Firestore(共有テストグループ`FMXRZYW7`)に対し旅行を
      1件作成し、しおり項目カード内の`.btn-secondary`の実測サイズを確認した
      ところ`{ width: 132, height: 36 }`(2026-08-21)、意図通り36px前後に
      縮小されていることを確認した。スクリーンショットでも、以前より控えめな
      見た目になっていることを目視確認した。

## 90. 足あと密度を増やす
- [x] (S) `85`で連結線を削除した後、足あと3件だけでは「足跡が少なすぎます」
      との指摘(2026-08-21)。`src/views/itinerary.js`の`buildTrailSvg()`内、
      `createFootprintTrail(Math.random, { footprintCount: 3 })`の
      `footprintCount`を6に増やした。`src/footprintTrail.js`側の
      デフォルト値(3)・既存のvitestテスト(「footprintCountを省略すると
      3件になる」)は変更していない(呼び出し側の明示指定のみ変更のため)。

      Playwrightで、しおりタブに実Firestore(共有テストグループ
      `FMXRZYW7`)上で項目を追加し、`.timeline-trail`内の`<g>`要素数が
      6件になっていることを確認した(2026-08-21)。

## 92. 移動前に時間矛盾の確認ダイアログを出す
- [x] (S) `87`実装時の警告(`.item-time-warning`)は「移動を確定させた後の
      再描画で気づく」方式だったが、人間から「87について、移動ボタンを
      押したときに、移動を起こす前に警告を出してほしい」とのフィードバック
      (2026-08-21)。`src/views/itinerary.js`の`moveItem`を、Firestoreへの
      書き込み前に移動後の並び(`reordered`)を対象に時刻の前後矛盾を判定し、
      矛盾が生じる場合は`window.confirm()`(しおり項目削除時の確認
      (`onDeleteItemClick`)と同じ既存パターン)で確認してから書き込む形に
      変更した。キャンセルした場合は書き込みを一切行わない(移動前の状態の
      まま)。矛盾があっても移動自体はブロックしない`87`の要件は維持し、
      「確認して続行できる」ダイアログにした。

      矛盾判定ロジックは`renderItems`内(移動後の表示バッジ用、`87`)と
      `moveItem`(移動前の確認用、本タスク)の両方で必要になったため、
      `findTimeInconsistentIds(orderedItems)`という共通関数に切り出した
      (`renderItems`側の重複していたロジックもこちらへ統一)。

      Playwrightで、実Firestore(共有テストグループ`FMXRZYW7`)に対し旅行を
      1件作成し、9:00「朝食」・12:00「昼食」・時間未設定「自由時間」を追加
      した状態で「昼食」を▲で上へ移動する操作を3パターン検証した
      (2026-08-21): (a)矛盾が生じる移動でダイアログが表示されること、
      (b)ダイアログをキャンセル(dismiss)すると並び順が変化しないこと、
      (c)ダイアログを続行(accept)すると実際に並び順が変わり、移動後の
      表示にも警告バッジが1件表示されること、をすべて確認した。
      スクリーンショットで警告表示を目視確認した。

      375px幅で全7タブとも横スクロールが発生しないこと(回帰なし)、
      console/pageerrorが0件であることも確認した(89/90/92共通の検証、
      2026-08-21)。`npm run check`(lint・test、vitest47件)成功。検証で
      作成したFirestore上のトリップ1件はサブコレクションを作成していない
      ため追加クリーンアップ不要、名前を「[検証用/削除不可] evolve
      cycle7 89/90/92検証で作成」に更新済みの状態で共有テストグループ
      `FMXRZYW7`内に残置。

## 91. 濃紺グラデーションの廃止
- [x] (M) 人間から「全体的にパステルオレンジにするなら青色いらなくない？」
      とのフィードバック(2026-08-21)。`83`実装時点では、ヘッダー・暗色
      カード(雑多メモ/企画メモ)・固定操作バーで使う濃紺グラデーション
      (`--color-primary-dark`実質ただの青`#1d4ed8`/`--color-primary-deep`
      藍系濃紺`#16234a`)は「暖色パレットの中の意図的なアクセント」として
      肯定的に評価され維持していたが、今回その判断を覆し暖色化する方針で
      人間と合意した。着手前にAskUserQuestionで代替案の方向性を確認し、
      「暖色系の濃い色を新設(推奨)」を選択してもらった上で実装した
      (CLAUDE.mdの「新しい色が必要な場合は実装せず確認する」方針に沿った
      対応)。

      `styles/tokens.css`の`--color-primary-dark`/`--color-primary-deep`を
      廃止し、新しいトークン`--color-ember-deep`(`#3a2116`、焦げ茶〜
      マホガニー寄りの最も濃い色。白文字とのcontrast比は実測14.9)・
      `--color-ember`(`#8c4a24`、シナモン/テラコッタ寄りの明るい側。
      白文字とのcontrast比は実測6.73)に置き換えた。いずれもWCAG AA基準
      (4.5)を大きく上回る。`pages/shared.css`側の4箇所(`.page-header`
      135deg・`.card-dark`160deg・`.scratch-actions`100deg・
      `.card-dark-accent`160deg、いずれも既存の`--color-accent`(オレンジ)
      との組み合わせ)の`background-image`を新トークンへ置き換えた。角度は
      いずれも変更していない(`83`で確定した役割ごとの角度差別化はそのまま
      維持)。旧トークンの参照が他に残っていないことをリポジトリ全体で
      grep確認した。

      Playwrightで、実Firestore(共有テストグループ`FMXRZYW7`)に対し旅行を
      1件作成し、ヘッダー・雑多メモ(`.card-dark`)・企画メモ
      (`.card-dark-accent`)それぞれの`background-image`の計算値を確認し、
      いずれも旧来の`#1d4ed8`/`#16234a`(青系)を含まず、新トークンの
      `rgb(58, 33, 22)`/`rgb(140, 74, 36)`(および企画メモは
      `rgb(255, 140, 66)`との組み合わせ)になっていることを確認した
      (2026-08-21)。スクリーンショットでも、ヘッダー・雑多メモ・企画メモの
      いずれも青みが完全に無くなり、統一感のある暖色トーンになっていることを
      目視確認した。375px幅で全7タブとも横スクロールが発生しないこと
      (回帰なし)、console/pageerrorが0件であることも確認した。`npm run
      check`(lint・test、vitest47件)成功。検証で作成したFirestore上の
      トリップ1件はサブコレクションを作成していないため追加クリーンアップ
      不要、名前を「[検証用/削除不可] evolve cycle7 91検証で作成」に
